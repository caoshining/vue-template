...
let Regx = {
  ...
  * */
  ...
  checkIsPhone(phone){
    ...
    // return (/^((13[0-9])|(14[5,7,9])|(15[^4])|(18[0-9])|(17[0,1,3,5,6,7,8]))\\d{8}$/).test(phone)
  },
  ...
  //   }
  // },
  checkPassword(password,ref,target){
  //  let pw = /^(?![\d]+$)(?![a-zA-Z]+$)(?![!@~,:&*?_\#/=+^;%]+$)[\da-zA-Z!#$%^&*]{6,20}$/g;
  // let pw = /^[A-Za-z0-9!@~,:&*?_\#/=+^;%]{6,16}$/
  let pw = /^(?=.*\d)(?=.*[a-zA-Z])[@~,:&*?!_#/=+^\\;%]?.{6,16}$/
  let space = /\s/g
   let hanzi = /[\u4e00-\u9fa5]+/
   ...
   let len = password.length
      ...
      ref.focus()
      target.$vux.toast.text('密码中不能包含空格', 'middle')
      return false;
    } else if (hanzi.test(password)) {
      ref.focus()
      target.$vux.toast.text('密码中不能包含汉字', 'middle')
      return false ;
    }else if(!special.test(password)){
      target.$vux.toast.text('密码中不能包含特殊字符', 'middle')
      ref.focus()
      return false ;
    }else if (len < 6) {
      target.$vux.toast.text('您输入的密码不能低于6位', 'middle')
      ref.focus()
      return false;
    } else if (len > 16) {
      target.$vux.toast.text('您输入的密码不能超过16位', 'middle')
      // Toast.info('您输入的密码不能超过20位', 1,null,false);
      ref.focus()
      return false;
    }else if (!(pw.test(password))) {
      target.$vux.toast.text('密码为6-16的字母、数字组合', 'middle')
      ref.focus()
      return false;
    } else {
      return true
